...
ARG QGIS_VERSION=4.0.1
ARG PDAL_VERSION=2.10.1
## Pre-installed PDAL version in the qgissi image
...
COPY conf${CUDA_IMAGE:+/cuda}/xfce /files
COPY scripts /files
...
RUN export $(cat /etc/os-release | grep "^ID=debian" | xargs) \
  ...
  && apt-get -y install --no-install-recommends \
    ...
    pyqt6-dev-tools \
    ## QGIS 4 depends on libpython3.x (the shared Python runtime library)
    ## The following package is installed for a lightweight integration
    python3-pyqt6.qtqml \
...
RUN export QT_QPA_PLATFORM=offscreen \
  ...
  ## QGIS Processing Executor: Workaround for the missing --profiles-path option
  ## https://github.com/qgis/QGIS/issues/44783
  && mv ${HOME}/.local/share/QGIS/QGIS3 ${HOME}/.local/share/QGIS/QGIS4 \
  && mv ${HOME}/.local/share/QGIS/QGIS4/profiles/default/QGIS/QGIS3.ini \
    ${HOME}/.local/share/QGIS/QGIS4/profiles/default/QGIS/QGIS4.ini \
  ## Clean up
  && rm -rf /tmp/* \
  && rm -rf \
    ${HOME}/.cache/QGIS \
  ...
  && rm -rf /var/backups/skel/.local/share/QGIS/QGIS3
